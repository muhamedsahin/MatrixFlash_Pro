...
Matrix solve(const Matrix& left, const Matrix& rhs) {
    ...
    if (rhs.rows() != n) {
        throw std::invalid_argument("solve() requires the RHS rows to match the coefficient matrix size");
    }
    ...
    try {
        ...
        if (info != 0) {
            throw std::runtime_error("solve(): back-substitution failed");
        }
...
QRResult qr(const Matrix& matrix) {
    ...
    try {
        ...
        if (info != 0) throw std::runtime_error("qr(): Q generation failed");
...
SVDResult svd(const Matrix& matrix) {
    ...
    try {
        ...
        if (info != 0) throw std::runtime_error("svd(): singular value decomposition failed to converge");
...
EigenResult eigen(const Matrix& matrix) {
    if (matrix.rows() != matrix.cols()) {
        throw std::invalid_argument("eigen() requires a square matrix");
    }
    const std::size_t n = matrix.rows();
    if (n == 0) {
        EigenResult empty;
    ...
    try {
        ...
        if (info != 0) {
            throw std::runtime_error("eigen(): eigendecomposition failed (matrix may not be symmetric)");
        }
...
Matrix solve_least_squares(const Matrix& left, const Matrix& rhs) {
    if (rhs.rows() != left.rows()) {
        throw std::invalid_argument("solve_least_squares() requires matching rows between A and b");
    }
