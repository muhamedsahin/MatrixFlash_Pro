...
Matrix matmul_half(const Matrix& left, const Matrix& right) {
    if (left.cols() != right.rows()) throw std::invalid_argument("Matrix dimensions are incompatible for matmul_half");
    ...
    try {
        ...
        if (status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error("FP16 cublasGemmEx failed");
    } catch (...) {
...
Matrix matmul_double_accumulate(const Matrix& left, const Matrix& right) {
    if (left.cols() != right.rows()) throw std::invalid_argument("Matrix dimensions are incompatible for double matmul");
    Matrix output(left.rows(), right.cols());
    if (output.empty()) return output;
