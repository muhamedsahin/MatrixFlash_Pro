...
Tensor batch_matmul(const Tensor& left, const Tensor& right) {
    if (left.rank() != 3 || right.rank() != 3) throw std::invalid_argument("batch_matmul requires rank-3 tensors");
    const auto& a = left.shape();
    const auto& b = right.shape();
    if (a[0] != b[0] || a[2] != b[1]) throw std::invalid_argument("batch_matmul shape mismatch");
    ...
    const auto status = cublasSgemmStridedBatched(
        ...
        static_cast<int>(a[0]));
    if (status != CUBLAS_STATUS_SUCCESS) throw std::runtime_error("cublasSgemmStridedBatched failed");
    return output;
}
}