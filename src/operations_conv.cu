...
void check_cudnn(cudnnStatus_t status, const char* operation) {
    if (status != CUDNN_STATUS_SUCCESS) throw std::runtime_error(std::string(operation) + ": " + cudnnGetErrorString(status));
}
#endif
...
__global__ void pool2d_kernel(const float* input, float* output,
    ...
    output[index] = maximum ? result : (valid == 0 ? 0.0f : result / static_cast<float>(valid));
}

void validate_nchw(const Tensor& input) {
    if (input.rank() != 4) throw std::invalid_argument("CNN input must have shape [N,C,H,W]");
}
...
Tensor conv2d(const Tensor& input, const Tensor& weights, const Tensor& bias,
              std::size_t stride, std::size_t padding) {
    validate_nchw(input);
    if (weights.rank() != 4 || bias.rank() != 1) throw std::invalid_argument("conv2d expects weights [K,C,R,S] and bias [K]");
    const auto& x = input.shape();
    const auto& w = weights.shape();
    if (w[1] != x[1] || bias.shape()[0] != w[0] || stride == 0) throw std::invalid_argument("conv2d channel or stride mismatch");
    if (x[2] + 2 * padding < w[2] || x[3] + 2 * padding < w[3]) throw std::invalid_argument("conv2d kernel is larger than padded input");
...
Tensor max_pool2d(const Tensor& input, std::size_t kernel_size, std::size_t stride, std::size_t padding) {
    validate_nchw(input);
    if (kernel_size == 0 || stride == 0) throw std::invalid_argument("pooling kernel and stride must be positive");
    const auto& x = input.shape();
    if (x[2] + 2 * padding < kernel_size || x[3] + 2 * padding < kernel_size) throw std::invalid_argument("pooling kernel is larger than padded input");
#ifdef MATRIX_PRO_HAS_CUDNN
