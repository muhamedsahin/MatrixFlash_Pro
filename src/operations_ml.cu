...
Matrix elu(const Matrix& matrix, float alpha) {
    if (alpha < 0.0f) throw std::invalid_argument("ELU alpha must be non-negative");
...
Matrix batch_norm(const Matrix& matrix, float epsilon) {
    ...
    if (epsilon <= 0.0f) throw std::invalid_argument("Batch norm epsilon must be positive");
    Matrix output(matrix.rows(), matrix.cols());
...
Matrix layer_norm(const Matrix& matrix, float epsilon) {
    if (matrix.empty()) return Matrix(matrix.rows(), matrix.cols());
    if (epsilon <= 0.0f) throw std::invalid_argument("Layer norm epsilon must be positive");
    Matrix output(matrix.rows(), matrix.cols());
...
Matrix dropout(const Matrix& matrix, float probability, unsigned long long seed) {
    if (probability < 0.0f || probability >= 1.0f) throw std::invalid_argument("Dropout probability must be in [0, 1)");
